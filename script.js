...
scene.background = new THREE.Color(0xf0fdfd);
...
const ambientLight = new THREE.AmbientLight(0xff2fff, 3);
...
loader.load("assets/ghana_organics_can_drink.glb", function (gltf) {
  model = gltf.scene;
  model.traverse((node) => {
    if (node.isMesh) {
      if (node.material) {
...
const scanSound = new Audio("./assets/sfx.mp3");
